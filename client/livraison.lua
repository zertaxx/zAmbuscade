...
local function menuDelivery()
    local main = RageUI.CreateMenu('Livraison', 'Ambuscade')
    local sub_main = RageUI.CreateSubMenu(main, 'Livraison', 'Ambuscade')
    local sub_three = RageUI.CreateSubMenu(main, 'Livraison', 'Ambuscade')
        ...
        Citizen.Wait(0)
            ...
            for k,v in pairs(Config.deliveryItems) do
                RageUI.ButtonWithStyle(v.label,nil, {RightLabel = "~o~"..v.prix.."$ Entreprise"}, true, function(Hovered, Active, Selected)
                    if Selected then
                        TriggerServerEvent('zAmbuscade:deliveryAdd', v.prix, v.name)
                    end
                end)
            end
...
local function menuRec()
    local menureco = RageUI.CreateMenu("Recolte", "Ambuscade")
            ...
            if PanelChargement == true then
                RageUI.PercentagePanel(Percentage or 0.0, "Chargement ... ("..math.floor(Percentage * 100).."%)", "", "",  function(Hovered, Active, Percent)
                    if Percentage < 1.0 then
                        ...
                        ESX.ShowNotification("<C>~r~Ambuscade</C>\n~s~Récolte en cours")
                        TriggerServerEvent("zAmbuscade:recolte", "menulivraison")
