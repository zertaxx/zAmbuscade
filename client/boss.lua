...
local function menuBoss()
    local menuBossP = RageUI.CreateMenu("Actions Patron", "~r~Ambuscade")
    ...
    while menuBossP do
        ...
        RageUI.IsVisible(menuBossP, true, true, true, function()
            ...
            if AmbuscadeMoney ~= nil then
                RageUI.Separator("~b~Argent de société : ~s~"..AmbuscadeMoney.."$")
            end
            ...
            RageUI.ButtonWithStyle("Retirer de l'argent",nil, {RightLabel = "→→"}, not cooldown, function(Hovered, Active, Selected)
                if Selected then
                    ...
                    if amount == nil then
                        RageUI.Popup({message = "<C>~b~[~b~Entreprise ~s~: ~r~Ambuscade~b~]\n~r~Montant invalide"})
                    else
                        TriggerServerEvent("zAmbuscade:retraitentreprise", amount)
                        refreshMoney()
                        coolcoolmec(3500)
                    end
                end
            ...
            RageUI.ButtonWithStyle("Déposer de l'argent",nil, {RightLabel = "→→"}, not cooldown, function(Hovered, Active, Selected)
                if Selected then
                    ...
                    if amount == nil then
                        ...
                        TriggerServerEvent("zAmbuscade:depotentreprise", amount)
                        refreshMoney()
                        coolcoolmec(3500)
                    end
                end
           ...
           RageUI.ButtonWithStyle("Accéder aux actions de Management",nil, {RightLabel = "→→"}, not cooldown, function(Hovered, Active, Selected)
                if Selected then
                    TriggerEvent('esx_society:openBossMenu', 'ambuscade', function(data, menu)
                        menu.close()
                    end, {wash = false})
                    RageUI.CloseAll()
                end
...
function refreshMoney()
	ESX.TriggerServerCallback('zAmbuscade:getSocietyMoney', function(money)
		UpMoneyAmbuscade(money)
	end)
end

function UpMoneyAmbuscade(money)
    AmbuscadeMoney = ESX.Math.GroupDigits(money)
end
