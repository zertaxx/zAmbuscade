...
local function menuCoffre()
    local menuCoffreP = RageUI.CreateMenu("Coffre", "~r~Ambuscade")
...
function menuCoffreRetirer()
    local menuCoffre = RageUI.CreateMenu("Coffre", "~r~Ambuscade")
    menuCoffre:SetRectangleBanner(Config.ColorMenuR, Config.ColorMenuG, Config.ColorMenuB, Config.ColorMenuA)

    ESX.TriggerServerCallback('zAmbuscade:getStockItems', function(items) 
    itemstock = items
                        ...
                        for k,v in pairs(itemstock) do 
                            ...
                            RageUI.ButtonWithStyle(v.label, nil, {RightLabel = v.count}, true, function(Hovered, Active, Selected)
                                if Selected then
                                    ...
                                    TriggerServerEvent('zAmbuscade:getStockItem', v.name, tonumber(count))
                                    RageUI.CloseAll()
                                end
                            end)
                        end
...
function menuCoffreDeposer()
    ...
    ESX.TriggerServerCallback('zAmbuscade:getPlayerInventory', function(inventory)
                         ...
                         local item = inventory.items[i]
                            if item.count > 0 then
                                    RageUI.ButtonWithStyle(item.label, nil, {RightLabel = item.count}, true, function(Hovered, Active, Selected)
                                            ...
                                            TriggerServerEvent('zAmbuscade:putStockItems', item.name, tonumber(count))
                                            RageUI.CloseAll()
                                        end
                                    end)
                                end
