...
Citizen.CreateThread(function()
    ...
    AddTextComponentString("Ambuscade")
...
local function MenuInterac()
    local main_kfc = RageUI.CreateMenu('Ambuscade', '~r~Intéractions Ambuscade')
    local ano_kfc = RageUI.CreateSubMenu(main_kfc, 'Ambuscade', '~r~Intéractions Ambuscade')
    ...
    ano_kfc:SetRectangleBanner(Config.ColorMenuR, Config.ColorMenuG, Config.ColorMenuB, Config.ColorMenuA)
                            ...
                            if (GetOnscreenKeyboardResult()) then
                                ...
                                if result then
                                    ...
                                    if player ~= -1 and distance <= 3.0 then
                                        TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_kfc', ('ambuscade'), montant)
        ...
        RageUI.IsVisible(ano_kfc, true, true, true, function()
            ...
            RageUI.List('Vos annonces', MenuDeOuf.action, MenuDeOuf.list, nil, {RightLabel = ""}, not cooldown, function(Hovered, Active, Selected, Index)
                if (Selected) then
                    if Index == 1 then
                   TriggerServerEvent('zAmbuscade:Annonce', true, false, false, false, false)
                    coolcoolmec(35000)
                    elseif Index == 2 then
                    TriggerServerEvent('zAmbuscade:Annonce', false, true, false, false, false)
                    coolcoolmec(35000)
                    elseif Index == 3 then
                    TriggerServerEvent('zAmbuscade:Annonce', false, false, true, false, false)
                    coolcoolmec(35000)
                    elseif Index == 4 then
                    TriggerServerEvent('zAmbuscade:Annonce', false, false, false, true, false)
                    coolcoolmec(35000)
                    elseif Index == 5 then
                    TriggerServerEvent('zAmbuscade:Annonce', false, false, false, false, true)
                    coolcoolmec(35000)
                    elseif Index == 6 then
                    local anomsg = KeyboardInput("Votre annonce", "", 100)
                    ExecuteCommand("ambuscade "..anomsg)
                    coolcoolmec(35000)
                    end
                    Wait(5)
                end
...
Keys.Register('F6', 'Ambuscade', 'Ouvrir le menu Ambuscade', function()
		...
		if isToHouse == true then
			...
			if GetDistanceBetweenCoords(px,py,pz, GetEntityCoords(GetPlayerPed(-1),true)) < 3 then
				...
				if IsControlJustPressed(1,38) then
                    ESX.TriggerServerCallback('zAmbuscade:livraisonRequire', function(item)
                        if item then	
                            ...
                            TriggerServerEvent("zAmbuscade:GiveCash", gainsMission, gainsEntreprise)
